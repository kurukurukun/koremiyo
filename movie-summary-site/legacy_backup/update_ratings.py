...
def get_eiga_score(title):
    try:
        ...
        # Eiga.com search result rating typically appears as <p class="rating-star val35 small">3.7</p>
        match = re.search(r'class="rating-star val\d* small">([\d\.]+)<', html)
        if not match:
            # Fallback for movie detail page or other structures
...
def main():
    print("=== アカデミー賞 作品賞 評価スクレイピング開始 ===")
    
    # 1. 既存の academy_data.js を読み込む
    with open(FILE_PATH, "r", encoding="utf-8") as f:
        content = f.read()

    # 変数の宣言部分と配列部分を分離する
    match = re.search(r'const academyWinners = (\[.*?\]);', content, re.DOTALL)
    if not match:
        print("エラー: js/academy_data.js の形式が読み取れません。")
        return

    json_str = match.group(1)
    # コメント行を削除
    json_str = re.sub(r'//.*', '', json_str)
    # JSのオブジェクトをJSONに変換するための簡易的な置換
    ...
    try:
        ...
        print("JSONのパースに失敗しました。ファイルが正しい形式か確認してください。", e)
        return
    ...
    print("=== 完了しました！ academy_data.js が完全に更新されました ===")
